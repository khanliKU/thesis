function result = simulateValveDynamics(...
var_min_air_gap,...
var_c_r        ,...
var_c_a        ,...
var_h_C        ,...
...
for i = 2:t_max
    ...
    if (x(i) == (var_h_air - var_min_air_gap) || t(i) >= tao_open ) && ~close_valve
        signal(i) = 0;
        close_valve = 1;
    else
        signal(i) = signal(i-1);
...
if isempty(orifice_opening_t)
    ...
    total_flow  = sum(m_dot_cham_d)*dt
end
...
if length(t) == t_max
    ...
    result.m_dot_cham_d = downsample(movmean(m_dot_cham_d(1:last_iteration),reduce_factor),reduce_factor);                               
