...
range_of_motion = 3e-3

## choke_point
choke_point_orifice_A = goal_A*np.sqrt(goal_Cd/Cd_cylinder) #m2
choke_point_orifice_h = range_of_motion #m
choke_point_orifice_r = np.ceil(choke_point_orifice_A/(2*np.pi*choke_point_orifice_h) * 1e4) / 1e4 #m

## outlet
outlet_max_r = choke_point_orifice_r
outlet_max_A = np.pi * outlet_max_r**2
print(outlet_max_A / choke_point_orifice_A)
if outlet_max_A < choke_point_orifice_A:
    raise "Choke point radius is too small"

## main_plunger
main_plunger_orifice_r = 1e-3 #m
main_plunger_r = np.floor((choke_point_orifice_r * 2) * 1000 ) / 1000 #m
main_plunger_tower_A = np.pi*main_plunger_r**2 #m2
main_plunger_tower_h = 10e-3 #m
main_plunger_work_volume = range_of_motion * main_plunger_tower_A
